...
def light_prop_gen():
    ...
    # create source field
    field_s1, xc_s1 = source_field(n_s1, x_s1, zo)
    ...
    # skip over propagation if too slow
    if False:
    ...
    pc_var = data * qe**2 / del_t**2
    q_n = np.sqrt(kb * temp * cap / qe**2)
    data2 = (eff * data) + np.random.poisson(pc_var) + np.random.normal(0, q_n, size=[n_sensy, n_sensx])
    ...
    ax[1].imshow(data2, extent=[scx_min, scx_max, scy_max, scy_min])
    plt.show()
...
def source_field(n, sz, z):
    ...
    field_s[int(n / 2), int(n / 2)] = lumin
    field_s[int(n * 1.1 / 2), int(n * 1.1 / 2)] = lumin
    field_s[int(n / 4), int(n / 2)] = lumin
    field_s[int(n / 2), int(n / 4)] = lumin
    field_s[int(n * 1.1 / 2), int(n/4)] = lumin
...
def zern_phase_scrn(ro, d, nn, zern_mx, x_crd, windx, windy, boil, deltat):
    ...
    for xx in np.arange(0, zern_mx-1):
        for yy in np.arange(0, zern_mx-1):
            idsi[xx,yy] = np.sum(np.sum(np.fft.fftshift(np.fft.ifft2(fft_mat[:,:,xx] * np.conjugate(fft_mat[:,:,yy])))
...
def fresnel_prop2(field_s, xms, yms, r_sz, zo, lam, arr_sz):
    ...
    for ii in range(arr_sz):
        for jj in range(arr_sz):
            field_r[ii, jj] = np.sum(np.sum(field_s * quadrtc * np.exp(-1j * 2 * np.pi * (xms * rc[jj] + yms * rc[ii]))))
...
def rayleigh_sommerfeld_prop(field_s, lam, zo, xs_crd, n_r, sz_r, xc, yc):
    ...
    for ii in range(np.size(yr_crd)):
        for jj in range(np.size(xr_crd)):
            ...
            field_r[ii, jj] = np.sum(np.sum(np.multiply(field_s, np.exp(1j * 2 * np.pi * r / lam) / (r**2 * lam * 1j))))
